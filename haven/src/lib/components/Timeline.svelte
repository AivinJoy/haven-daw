...
<script lang="ts">
    ...
    function handleWheel() {
        ...
        wheelTimeout = setTimeout(() => isWheeling = false, 500);
    }
    
    $effect(() => {
        // ALWAYS track dependencies first so Svelte never drops reactivity
        const playheadPx = currentTime * PIXELS_PER_SECOND * zoomMultiplier;
        const cw = containerWidth;

        if (!trackContainer || isScrubbing || cw === 0 || isAutoScrollSuspended) return;

        const currentScrollPx = trackContainer.scrollLeft;
        let targetScroll = -1;

        if (playheadPx > currentScrollPx + cw) {
            targetScroll = playheadPx - 50; 
    ...
    function handleScroll() {
        ...
        requestAnimationFrame(() => {
            ...
            // Only suspend if the user is actively dragging the scrollbar or using the mouse wheel/trackpad.
            // Completely ignores invisible layout shifts caused by deleting clips!
            if (isPointerDown || isWheeling) {
                suspendAutoScroll();
            }
